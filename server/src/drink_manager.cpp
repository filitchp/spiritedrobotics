...
void DrinkManager::updateLloyd(string name, string drink, string status){

  try { 
  std::string query_string = string("?") 
    + "name=" + name
    + "&drink=" + drink
    + "&status=" + status;
  ...
  tcp::resolver::query query("spiritedrobotics-lloyd.herokuapp.com", "http");
  ...
  request_stream << "GET /" << query_string << " HTTP/1.0\r\n";
  request_stream << "Host: " << "spiritedrobotics-lloyd.herokuapp.com"  << "\r\n";
...
void DrinkManager::timerEventOperation()
{
  ...
  switch (currentEvent.getType())
  {
    ...
    case EventTypeIngredient:
        ...
        break;
    case EventTypeLloyd:
        cout << "Lloyd Event " << currentEvent.getStatus() << endl;
        updateLloyd(currentEvent.getName(), currentEvent.getDrink(), currentEvent.getStatus());
        break;
...
int DrinkManager::approveOrder(string drinkKey, string customerName, unsigned timestamp)
{
  ...
  mEventQueue.push_back(Event(EventTypeLloyd, theOrderToMake.getCustomerName(),  theOrderToMake.getDrinkKey(), string("WARMUP") ));
  ...
  mEventQueue.push_back(Event(EventTypeLloyd, theOrderToMake.getCustomerName(),  theOrderToMake.getDrinkKey(), string("POURING") ));
  ...
  mEventQueue.push_back(Event(EventTypeLloyd, theOrderToMake.getCustomerName(),  theOrderToMake.getDrinkKey(), string("IDLE") ));
